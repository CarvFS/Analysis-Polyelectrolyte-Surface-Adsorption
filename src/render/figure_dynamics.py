...
| Date: 2023-11-24
| Description: Module to render figures for time series data.
...
def opes_dynamics(
    df: pd.DataFrame, cv: str = "dist_chain.z", dir_fig: str = "figures"
) -> plt.Figure:
    """
    ...
    ax5.set_ylabel(r"$n_\mathrm{eff}$", labelpad=10)
    ...
    ax6.set_ylabel(r"$n_\mathrm{ker}$", labelpad=10)
    ...
    fig.savefig(
        f"{dir_fig}/{fname}.pdf", dpi=1200, bbox_inches="tight", transparent=True
    )
...
def cv_replica_dynamics(
    dfs: list[pd.DataFrame], cv: str = "dist_chain.z", dir_fig: str = "figures"
) -> plt.Figure:
    """
    Render a figure of the collective variable dynamics for each replica. This
    figure should be used to assess the sampling of the collective variable
    space across replicas. The figure contains 8 subplots, one for each replica.

    Parameters
    ----------
    dfs : list[pd.DataFrame]
        List of DataFrames containing the collective variable dynamics for each
        replica.
    cv : str, optional
        Name of the collective variable, by default "dist_chain.z"
    dir_fig : str, optional
    ...
    -------
    plt.Figure
        Figure of the collective variable dynamics for each replica.
    """
    ...
    for i in range(8):
        ...
        ax.scatter(dfs[i]["time"] / 1e3, dfs[i][cv], s=1, alpha=0.2)
